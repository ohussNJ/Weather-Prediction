public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        int k=0;
        int n=5000;
        int []forecast=new int [n];
        for(int i=0; i<drywet.length; i++) {
         if(longitute==drywet[i][0] && latitude==drywet[i][1]) {
          forecast[k]=(drywet[i][month+1]<0.5?DRY:WET);
          k++;
         }
        }
        for(int i=0; i<wetwet.length; i++) {
         if(longitute==wetwet[i][0] && latitude==drywet[i][1]) {
          forecast[k]=(wetwet[i][month+1]>=0.5?WET:DRY);
          k++;
         }
        }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for(int i=0; i<forecast.length; i++) {
         if(mode==forecast[i]) {
          length++;
          CurrDaysSpell=length;
         } else {
          length=0;
         }
         MaxDaysSpell=CurrDaysSpell;
        }
        return MaxDaysSpell;
    }
